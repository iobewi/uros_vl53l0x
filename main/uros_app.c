...
// ---- Configuration ----
#define DOMAIN_ID 42
#define TIMER_PERIOD_MS 100   // 10 Hz; increase if XRCE payload becomes heavy
...
// Densified scan: 2° bins (~180). Adjust as needed.
#define ANGLE_MIN   (-(float)M_PI)
#define ANGLE_INC   (DEG2RAD(2.0f))
#define N_BINS      ((int)lroundf((2.0f * (float)M_PI) / ANGLE_INC))

static const char *SCAN_FRAME = "base_link";

// Example angles (side-focused). Replace with your real layout.
static const float TOF_ANGLES_RAD[TOF_COUNT] = {
    DEG2RAD(-120.0f),
    DEG2RAD(-90.0f),
    DEG2RAD(-60.0f),
    DEG2RAD(-30.0f),
...
static void timer_callback(rcl_timer_t *timer, int64_t last_call_time)
{
    ...
    scan_builder_fill(&scan_msg, &scan_cfg, snap);
...
static void micro_ros_task(void *arg)
{
    ...
    scan_cfg.tof_angles_rad = TOF_ANGLES_RAD;
    ...
    ESP_LOGI(TAG_TASK, "micro-ROS up: node=%s topic=%s period=%dms", NODE_NAME, TOPIC_NAME, TIMER_PERIOD_MS);
